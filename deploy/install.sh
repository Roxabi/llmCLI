...
echo "  3. systemctl --user start llmcli               # M₂ local hybrid proxy (llm-worker)"
echo "  4. systemctl --user start llmcli-nats-worker   # worker (llm-worker hosts only)"
echo ""
echo "  M₁ cloud gateway (LiteLLM proxy + xAI/Grok + Fireworks forwarders) is deployed"
echo "  from roxabi-factory now (make converge on M₁). xAI OAuth is still minted here:"
echo "    llmcli xai login   # run on M₁; writes ~/.roxabi/llmcli/credentials/ (per-host grant)"
