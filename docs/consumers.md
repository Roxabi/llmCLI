...
native Anthropic-compatible endpoint with extended thinking enabled. The alias is
**keyless** — `FIREWORKS_API_KEY` stays on the server inside `~/.roxabi/llmcli/env/proxy.env`
and is never required in the client environment.
